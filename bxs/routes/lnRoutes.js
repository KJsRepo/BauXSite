...
router.get('/deposit', (req, res) => {
  ...
  res.render('maintemplate', {contentpartial: 'deposit.ejs', data: app.data, user: req.session.userdata})
})


router.post('/deposit', (req, res) => {

  ln.createInvoice(req.session.userdata.username, req.body.amount, req.session.userdata._id)
    .then ((invoiceData) => {
      ...
      res.render('maintemplate', {contentpartial: 'invoice.ejs', data: app.data, user: req.session.userdata})
    })
    .catch((err) => {
      req.session.error = err
      console.log(err);
      req.session.save((err) => { if (err) { controller.logError(err) } else { return res.redirect('/h/deposit') }})
    })
...
router.get('/withdraw', (req, res) => {
  ...
  res.render('maintemplate', {contentpartial: 'withdraw.ejs', data: app.data, user: req.session.userdata})

})

router.post('/withdraw', (req, res) => {

  ln.payLNInvoiceFromUser(req.body.request_hash, req.session.userdata._id)
    .then((result) => {
